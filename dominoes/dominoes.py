...
class Dominoes:
    ...
    def prevent_duplicates(self, some_knucks: list[list[int]]) -> list[list[int]]:
        ...
        bricks_without_duplic = []
        [bricks_without_duplic.append(x) for x in some_knucks if x not in bricks_without_duplic]
        self.bricks = bricks_without_duplic

        return self.bricks

    def take_seven_bricks(self, dom_set: list[list[int]]) -> tuple[list[list[int]], list[list[int]]]:

        second_set = random.sample(dom_set, 7)
        self.bricks = [x for x in dom_set if x not in second_set]

        return self.bricks, second_set
...
""")
        ...
        for j in range(6, -1, -1):
            for i in range(len(pl_set)):
                if [j, j] == pl_set[i]:
                    ...
                    pl_set.pop(i)
                    stat = PC
                    self.bricks = dominoes.output_main(dom_set, comp_set, pl_set, dom_snake, stat)
                    done = True
                    break
    ...
    def put_knuckle_onto_snacke(self, dom_set: list[list[int]], ini_set1: list[list[int]], ini_set2: list[list[int]],
        ...
        if stat == player:
            self.bricks = dominoes.output_main(dom_set, ini_set2, ini_set1, dom_snake, PC)
            return dom_set, ini_set2, ini_set1, dom_snake
        else:
            self.bricks = dominoes.output_main(dom_set, ini_set1, ini_set2, dom_snake, player)
...
if __name__ == '__main__':
    ...
    while not done_first_step:
        ...
        dominoes_set = dominoes.prevent_duplicates(dominoes_set)

        dominoes_set, player_dominoes = dominoes.take_seven_bricks(dominoes_set)

        dominoes_set, computer_dominoes = dominoes.take_seven_bricks(dominoes_set)
